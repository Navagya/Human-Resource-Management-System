...
function AdminLeavePage() {
    ...
    const updateStatus = async(id,status)=>{
        try{
            await axios.put(`http://localhost:5000/api/leaves/update/${id}`,
                {status},
                {headers:{Authorization:`Bearer ${token}`}}
            );
            fetchLeaves();
  ...
  return (
    <div className="p-6 text-white">
        <h2 className="text-2xl font-semibold mb-4">All Leaves Requests</h2>
        {leaves.length===0?(
            <p>No leaves found.</p>
        ):(
            <div className="space-y-3">
                {leaves.map((l)=>(
                    <div key={l._id} className="bg-zinc-800 p-3 rounded">
